...
<h2 class="text-lg font-medium mr-auto">
    {{ __('app.documents') }} - {{__('app.edit_tag')}}
</h2>
<!-- END: Breadcrumb -->
@endsection
...
<div class="intro-y p-5 mb-5 flex flex-wrap items-center border-b">
    ...
    <div class="-intro-x breadcrumb mr-auto hidden sm:flex">
        ...
        <a href="{{ route('documents.index') }}" class="breadcrumb">{{ __('app.documents') }}</a>
        <i data-feather="chevron-right" class="breadcrumb__icon"></i> 
        <a href="{{ route('documents.tags') }}" class="breadcrumb">{{ __('app.tag_management') }}</a>
        <i data-feather="chevron-right" class="breadcrumb__icon"></i> 
        <a href="#" class="breadcrumb--active">{{__('app.edit_tag')}}</a>
    </div>
...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y box mt-5 col-span-12 lg:col-span-12">
        ...
        <div class="p-5">
            <form action="{{ route('documents.tags.save') }}" method="POST" id="create_form">
                @csrf
                <input type="hidden" name="id" value="{{ $tag->id }}" />
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <label class="col-span-12 lg:col-span-3">{{__('app.tag_name')}}:</label>
                    <div class="col-span-12 lg:col-span-9">
                        <input type="text" placeholder="{{__('app.tag_name')}}..." name="tag_name" id="tag_name"
                            class="input w-full border" value="{{ $tag->tag_name }}">
                    </div>
                </div>
                <div class="grid grid-cols-12 gap-6 mt-5">
                    ...
                    <div class="col-span-12 lg:col-span-9">
                        <select name="color" class="input border mr-2 w-full" id="color_selector">
                            ...
                            @foreach(config('constants.colors') as $color)
                                <option value="{{$color['value']}}" @if($tag->color == $color['value']) selected @endif>{{ $color['value'] }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
