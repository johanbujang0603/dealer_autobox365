...
class LeadDocumentsController extends Controller
{
    ...
    public function convert_upload(Request $request)
    {
        if ($file = $request->file('file')) {

            $path = "documents/leads";
            ...
            $document->upload_path = $path . '/' . $upload_file_name;
            $document->kinds = 'lead';
            $document->parent_inventories = null;
            $document->parent_customers = null;
            $document->parent_users = null;
            $document->parent_leads = $lead_id;
            $document->parent_locations = null;
    ...
    {
        $documents = Document::where('parent_leads', "LIKE", "%" . $id . "%")->get();
        $html_str = "";
